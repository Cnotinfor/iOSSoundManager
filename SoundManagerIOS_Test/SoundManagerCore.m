...
- (Buffer*) loadWave:(NSString*)resource path:(NSString*)path
{
	...
	CFURLRef soundURL = (CFURLRef)[[NSURL fileURLWithPath:r] retain];
...
{
	NSString* prefix = [[NSString alloc] stringByAppendingFormat:@"%d", instrument];
...
{
	NSString* prefix = [[NSString alloc] stringByAppendingFormat:@"%d_%d", instrument, tempo];
...
{
	[SoundManagerCore checkOpenALErrors];
	NSLog(@"[SoundManagerCore::playMelody]");
	...
	for (int i = 0; i < _numberOfNotes; i++)
	{
		...
		Buffer* buffer = [self noteBuffer:[melody instrument] tempo:[melody tempo] height:[currentNote note] duration:[currentNote duration] octave:[currentNote octave]];
	...
	for (int i = 0; i < NUM_AVAILABLE_RHYTHMS; i++)
	{
		int variation = [[[melody rhythms] objectAtIndex:i] intValue];
		if( variation > 0 )
		{
			// Get rhythm buffer
			Buffer* buffer = [self rhythmBuffer:i tempo:[melody tempo] variation:variation];
...
- (void)stopMelody
{
	...
	alSourceUnqueueBuffers( _melodySources[0], _numberOfNotes ,_melodyBuffers );
	
	// emit signal finish melody
	
	// TODO atenção com acesso simultaneo entre stop e play
}
...
{
	return [[NSString alloc] initWithFormat:@"pause_%d_%d", tempo, duration];
}
...
{
	return [[NSString alloc] initWithFormat:@"%d_%d_%d", rhythm, tempo, variation];
}
...
{
	return [[NSString alloc] initWithFormat:@"%d_%d_%d_%d_%d", instrument, tempo, duration, octave, height];
}
